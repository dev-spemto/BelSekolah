...
class MainController extends Controller
{
    public function index()
    {
        ...
        // --- FILTER PRATINJAU MATRIX BERDASARKAN MODE AKTIF ---
        $matrixQuery = Schedule::query();

        if ($activeMode !== 'regular') {
            // Jika mode khusus (asts, asas, puasa, dll), filter HANYA varian mode tersebut
            $matrixQuery->where('variant', $activeMode);
        } else {
            // Jika mode Reguler, tampilkan varian default, jumat_jamaah, dan jumat_ringkas
            $matrixQuery->whereIn('variant', ['default', 'jumat_jamaah', 'jumat_ringkas']);
        }

        $allSchedules = $matrixQuery->orderByRaw($dayOrder)
            ->orderBy('time', 'asc')
            ->orderBy('variant', 'asc')
            ->get();
    ...
    public function getNextSchedule()
    {
        ...
        foreach ($todaySchedules as $schedule) {
            ...
            // Jika jadwal berupa istirahat atau pulang, cek ketersediaan audio acak lagu nasional/daerah
            if ($audioLower === 'bel_istirahat.mp3' || stristr($schedule->event_name, 'istirahat')) {
                $randomAudio = $this->pickRandomNationalAudio('istirahat');
                if ($randomAudio && Storage::disk('public')->exists("sounds/{$randomAudio}")) {
                    $schedule->audio_file = $randomAudio;
                }
            }
            elseif ($audioLower === 'bel_pulang.mp3' || stristr($schedule->event_name, 'pulang')) {
                $randomAudio = $this->pickRandomNationalAudio('pulang');
                if ($randomAudio && Storage::disk('public')->exists("sounds/{$randomAudio}")) {
                    $schedule->audio_file = $randomAudio;
                }
            }
